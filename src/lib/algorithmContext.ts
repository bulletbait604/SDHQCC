...
export async function formatAlgorithmContextForPlatform(platformId: string): Promise<{
  ...
  const parts = [
    ...
    rec.keyChanges ? `Ranking / key changes: ${rec.keyChanges.slice(0, 900)}` : '',
    rec.editingTips ? `Editing tips: ${rec.editingTips.slice(0, 700)}` : '',
    rec.postingTips ? `Posting tips (times/frequency): ${rec.postingTips.slice(0, 700)}` : '',
    rec.titleTips ? `Title/hook tips: ${rec.titleTips.slice(0, 500)}` : '',
    rec.descriptionTips ? `Caption/description tips: ${rec.descriptionTips.slice(0, 500)}` : '',
  ].filter(Boolean)
