...
async function generateTagsWithGemini(description: string, platform: string, count: number): Promise<string[]> {
  ...
  try {
    ...
    console.log('[Tags] Calling Gemini API with model: gemini-flash-latest')
...
async function generateTags(description: string, platform: string, count: number): Promise<{ tags: string[], provider: string }> {
  ...
  try {
    console.log('[Tags] Generating tags with Gemini Flash Latest...')
    ...
    return { tags, provider: 'gemini-flash-latest' }
  } catch (error: any) {
...
export async function GET() {
  return NextResponse.json({ 
    message: 'Using Gemini Flash Latest for tag generation',
