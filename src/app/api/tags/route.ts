...
async function generateTagsWithGemini(description: string, platform: string, count: number): Promise<string[]> {
  ...
  try {
    ...
    console.log('[Tags] Calling Gemini API with model: gemini-2.5-flash-lite')
...
async function generateTags(description: string, platform: string, count: number): Promise<{ tags: string[], provider: string }> {
  ...
  try {
    console.log('[Tags] Generating tags with Gemini 2.5 Flash Lite...')
    ...
    return { tags, provider: 'gemini-2.5-flash-lite' }
  } catch (error: any) {
...
export async function GET() {
  return NextResponse.json({ 
    message: 'Using Gemini 2.5 Flash Lite for tag generation',
